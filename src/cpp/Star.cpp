...
Star::Star(Vector2 position) :
    m_position(position), m_numFrames(constants::STAR_FRAMES), m_currentFrame(0), m_animationTimer(0.0f),
    m_animationInterval(constants::STAR_ANIMATION_INTERVAL), m_collected(false), m_scale(constants::STAR_SCALE),
    m_speed(0.0f), m_rotation(0.0f), m_spinSpeed(0.0f) {
...
Star::~Star() { UnloadTexture(m_texture); }

auto Star::update() -> void { applyMovement(m_speed); }

auto Star::changeSpeed(float n) -> void { m_speed = n; }

auto Star::draw() -> void { DrawTextureEx(m_texture, m_position, constants::STAR_ROTATION, m_scale, WHITE); }

auto Star::getBoundingBox() const -> Rectangle {
    return {m_position.x, m_position.y, m_frameRec.width * m_scale, m_frameRec.height * m_scale};
}

auto Star::isCollected() const -> bool { return m_collected; }
...
auto Star::SpawnRandom(const Rectangle &spawnBounds,
...
auto Star::getCollectedCount() -> int { return s_collectedStars; }

auto Star::applyDashBoost(float dashBoost) -> void { m_position.x -= dashBoost; }

auto Star::applyMovement(float speed) -> void { m_position.x -= speed; }
