...
ReplaySystem::~ReplaySystem() { LOG("ReplaySystem destroyed"); }
...
auto ReplaySystem::saveToFile(const std::string &filename) -> void {
    ...
    auto newFilename = (replayDir / ("replay_" + std::string(buffer) + "_time" +
                                     std::to_string(static_cast<int>(finalTime)) + ".dat"))
                               .string();
...
auto ReplaySystem::isPlaybackFinished() const -> bool { return !m_playbackActive; }
