    ...
    : m_stateMachine(sm)
{
    ...
    try {
        ...
        // Example for loading codepoints (trimmed for brevity)
        const char *text = "Some UTF-8 text";
            ...
            try {
                ...
                m_clouds.push_back(std::make_unique<Cloud>(
                    fs::path("resources/clouds/Cloud_" +
                             ...
                             ".png"),
                    m_speed,
                    Random::get(constants::CLOUD_MIN_SCALE,
                                constants::CLOUD_MAX_SCALE),
                    config::SCREEN_WIDTH,
                    spawnY,
...
void GameScreen::spawnStars() {
    if (m_stars.size() < config::MAX_STARS && GetRandomValue(0, 100) < 100) {
        ...
        float spawnWidth = constants::CLOUD_SPAWN_MARGIN; // replaced 300.0f
...
void GameScreen::spawnPlatforms(std::vector<std::unique_ptr<Platform>>& platforms, bool bottom) {
    if (platforms.size() < config::MAX_PLATFORMS) {
        ...
        if ((bottom && (platforms.empty() || lastX <
            config::SCREEN_WIDTH - config::PLATFORM_WIDTH *
            constants::BOTTOM_PLATFORM_GAP_MULTIPLIER)) ||
            (!bottom && (platforms.empty() || lastX <
            config::SCREEN_WIDTH - config::PLATFORM_WIDTH *
            constants::TOP_PLATFORM_GAP_MULTIPLIER))) {
            try {
                int spawnY = bottom
                    ? Random::get(constants::PLATFORM_BOTTOM_MIN,
                                  config::SCREEN_HEIGHT -
                                  constants::PLATFORM_BOTTOM_MAX_OFFSET)
                    : Random::get(0,
                                  constants::PLATFORM_TOP_MAX);
                auto newPlatform = std::make_unique<Platform>(
                    m_speed, config::SCREEN_WIDTH, static_cast<float>(spawnY), 1.0f
                );
...
void GameScreen::update() {
    ...
    m_character->update(m_input_jump, m_input_dash,
                        m_platforms_bottom, m_platforms_top);

    for (auto it = m_stars.begin(); it != m_stars.end();) {
        (*it)->update();
        if (CheckCollisionRecs(m_character->getCollisionRect(),
                               (*it)->getBoundingBox())) {
            m_stars_collected++;
            Vector2 popPos { m_character->getCollisionRect().x,
                             m_character->getCollisionRect().y };
...
void GameScreen::render() {
    ...
    if (!m_character->isAlive()) {
        ...
        Color mainColor = RED;
        Vector2 pos {
            static_cast<float>(config::SCREEN_WIDTH / 2 - 100),
