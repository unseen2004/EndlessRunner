...
auto Character::update(bool input_jump,
    ...
    if (!was_grounded && m_is_grounded) m_jumps_left = config::JUMPS_NUMBER;
    handleJump(input_jump);
    float dt = GetFrameTime();
    if (m_dash_cooldown_timer > 0) m_dash_cooldown_timer -= dt;
    handleDash(input_dash);
    if (!m_is_grounded) applyGravity();
...
auto Character::draw() -> void {
    ...
    Rectangle dest = { m_position.x + origin.x, m_position.y + origin.y,
                       m_frame_rec.width, m_frame_rec.height };
...
auto Character::getDashBoost(float dt) const -> float {
            ...
            for (float offset = 0.0f; offset <= constants::GROUND_SCAN_RANGE; offset += constants::GROUND_STEP) {
                ...
                if (p->checkCollision({ sx, sy })) {
                    ...
                    while (candidateY - constants::GROUND_STEP > bottom) {
                        ...
                        if (!p->checkCollision({ ux, uy })) break;
...
auto Character::checkRightCollision(const std::vector<std::unique_ptr<Platform>>& platforms) -> bool {
    ...
    for (auto const& p : platforms) {
        ...
        if ((characterTop + headRegionHeight) < pt || characterTop > pb) continue;
        ...
        if (collidingSamples >= collThreshold) return true;
    }
    return false;
}
