...
Character::Character(std::filesystem::path path1, std::filesystem::path path2, std::filesystem::path path3, float x,
    ...
    : m_pathCharacter(path1), m_pathExplosion(path2), m_pathSound(path3), m_position{x, y},
...
void Character::dead() {
    InitAudioDevice();
    float frameWidth = (float)(m_explosion.width / config::NUM_FRAMES_PER_LINE);
    float frameHeight = (float)(m_explosion.height / config::NUM_LINES);
    ...
    while (active) {
        if (m_framesCounter > 2) {
            ...
            if (m_currentFrame >= config::NUM_FRAMES_PER_LINE) {
                m_currentFrame = 0;
                ++m_currentLine;

                if (m_currentLine >= config::NUM_LINES) {
                    m_currentLine = 0;
                    active = false;
                }
            }
