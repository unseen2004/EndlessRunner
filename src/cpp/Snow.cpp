...
Snow::Snow(int count, int screenWidth, int screenHeight)
    ...
    for (int i = 0; i < count; i++) {
        SnowParticle p;
        p.position = {
            static_cast<float>(GetRandomValue(0, m_screenWidth)),
            static_cast<float>(GetRandomValue(0, m_screenHeight))
        };
        p.speed = static_cast<float>(
            GetRandomValue(constants::SNOW_MIN_SPEED, constants::SNOW_MAX_SPEED)
...
auto Snow::draw() -> void {
    for (auto &p : m_particles) {
        DrawCircleV(
            p.position,
            constants::SNOW_PARTICLE_RADIUS,
